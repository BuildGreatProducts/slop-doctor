...
| Auth callbacks | `https://dashing-camel-39.convex.site/api/auth/callback/{github,google}` | `https://<prod-deployment>.convex.site/api/auth/callback/{github,google}` |
...
1. **You:** set the dev keys if you haven't:
   ...
   ```
   Repeat for `GEMINI_API_KEY`, `TYPESAFE_API_KEY`, `AUTH_GITHUB_ID`, `AUTH_GITHUB_SECRET`, `AUTH_GOOGLE_ID` and `AUTH_GOOGLE_SECRET`.
...
## 4. Production OAuth apps

**You:** create a second GitHub OAuth app and a second Google OAuth client for production:

- GitHub: homepage `https://<your-vercel-domain>`, callback `https://<prod-deployment>.convex.site/api/auth/callback/github`
- Google: authorised redirect URI `https://<prod-deployment>.convex.site/api/auth/callback/google`
...
The full list for `--prod`: `SITE_URL`, `AUTH_GITHUB_ID`, `AUTH_GITHUB_SECRET`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `FIRECRAWL_API_KEY`, `GEMINI_API_KEY`, `TYPESAFE_API_KEY`. Check with `npx convex env list --prod`.
...
3. **You:** deploy. Note the domain and, if it differs from what you used above, update `SITE_URL` and the OAuth homepages.

## 6. Smoke test production

1. **You:** open the live URL, sign in with GitHub and with Google.
