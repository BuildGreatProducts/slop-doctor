...
- **Academy referral:** the chart closes with a full-width `panel-grid` band, the same style as the chart's header band. This is a fourth permitted home for the paper grid (founder decision, 23 Sep 2026). It holds a `label-mono` eyebrow, a `headline-lg` headline, `body-lg` text in `on-surface-variant` on the 720px measure, and a `button-primary` link; the 160px pencil doctor sits on the right on desktop and is hidden under 1024px.
- **Sign-in popup:** the system `dialog` (native `<dialog>`, default 480px width), with the 40px logo mark, a `headline-md` title, the note in `body-sm` `on-surface-variant`, then `button-primary` "Continue with Google" and `button-ghost` "Close".
- **Example report** on the landing page: a `label-mono` label, the example share card in an `md`-cornered `outline-variant` hairline frame on the 720px measure, and a `body-sm` caption.
...
- **Treat popup:** the same 760px system `dialog` as the share popup. It shows a `headline-md` title, the intro in `body-md`, then the prompt in a `code` block (below), wrapped and capped at 12 lines with its own scroll. Then three `button-secondary` links, "Open Claude", "Open Codex" and "Open Cursor", with no one agent favoured, and `button-ghost` "Copy prompt". The help line sits beneath in `body-sm` `on-surface-variant`. The buttons stack full-width under 600px. On the chart, "Treat with your agent" is the `button-secondary` and "Get a second opinion" drops to `button-ghost` (founder decision, 24 Sep 2026).
- **`code` block:** Geist Mono at `body-sm` size, line height 1.6, normal case and tracking, `inverse-on-surface` on `inverse-surface`, `sm` corners, `md` padding. It is the one place mono sets full sentences, because it shows text that is meant to be pasted somewhere else.
