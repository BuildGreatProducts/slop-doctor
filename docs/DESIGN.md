...
- **Slop-o-meter** reuses the ladder shape: ten square `rung` segments. Filled segments use `inverse-surface` (not `primary`), empty ones are `rung-hatched`. The label beneath is `label-mono` (`SLOP INDEX 62/100`). The meter never takes the orange: during an examination the orange element is the scan rule, and on a chart it is the "Copy discharge papers" button.
...
- **`region-box`** is a 1px `pencil` hairline rectangle drawn around a region when its findings arrive. Square corners. No fill, no shadow. An inconclusive region uses a dashed 1px `outline` instead.
- **`region-tag`** pins to the region box's top-left corner: `label-mono` on `surface` with a 1px `pencil` border (`03 · PURPLE GRADIENT FEVER · P 0.91`).
- **Lab results** is a `list-item` list; each row has the symptom name in `label-md`, the probability in `label-mono`, and a `lab` or `exam` chip.
- **Severity** is carried by text and position, not colour: severe symptoms list first, and `error` is used only for real failures, never to decorate a diagnosis.
